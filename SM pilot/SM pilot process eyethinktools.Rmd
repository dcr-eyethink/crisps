...
```{r}
mitem <- read.csv("https://raw.githubusercontent.com/dcr-eyethink/crisps/refs/heads/master/SM%20pilot/memkey.csv")
...
```{r}
pirateye(pd, colour_condition = "design",x_condition = "variable",
         dv=c("react_filler","react_trees","share_filler",  "share_trees",
              "comment_made_filler",  "comment_made_trees"  ),violin = F,dots=F)
```
...
```{r message=FALSE, warning=FALSE}
pirateye(pd, colour_condition = "design",x_condition = "variable",
         dv=c("mem_rt_filler","mem_rt_trees"))
```

Behvaioural intentions
